...
onBeforeMount(async () => {
  ...
  if (!postDetail.value) {
    ...
    // If there is no cat data, it means the post was cached
    if (!postDetail.value!.cat) {
      await postStore.fetchPostDetail(postId, true)
    }
  }
...
<template>
  ...
  <LoadingSpinner v-else class="text-6xl h-app flex items-center justify-center w-full" />
</template>
